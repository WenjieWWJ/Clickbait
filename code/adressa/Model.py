...
class MMGCN(torch.nn.Module):
    def __init__(self, d_feat, b_feat, t_feat, edge_index, batch_size, num_user, num_item, aggr_mode, concate, num_layer, has_id, user_item_dict, dim_x, alpha):
        ...
        self.t_gcn = GCN(self.edge_index, batch_size, num_user, num_item, self.t_feat.size(1), dim_x, self.aggr_mode, self.concate, num_layer=num_layer, has_id=has_id, dim_latent=128)

    def forward(self, user_nodes, pos_item_nodes, neg_item_nodes):
        d_rep = self.d_gcn(self.d_feat, self.id_embedding)
        b_rep = self.b_gcn(self.b_feat, self.id_embedding)
        t_rep = self.t_gcn(self.t_feat, self.id_embedding)
        
        # pre_interaction_score
        pre_representation = (t_rep + d_rep)/2
        ...
        post_representation = (d_rep+b_rep+t_rep)/3
